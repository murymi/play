...
pub fn build(b: *std.Build) void {
    ...
    const c = b.option(bool, "ufala", "cheza na id");
    ...
    const rstep = RamdiskStep.create(b, target);
    //_ = rstep;
    ...
    const waa = b.step("waa", "fanya alalala");

    //rstep.step.dependOn(waa);

    waa.dependOn(&rstep.step);
...
const RamdiskStep = struct {
   ...
   // const Error = (error{EndOfStream} || File.ReadError || File.SeekError || Allocator.Error || File.WriteError || File.OpenError);
    ...
    ///
    fn writeRamdisk(comptime Usize: type, self: *RamdiskStep) !void {
        _ = self;
        _ = Usize;
    }
    ...
    ///     Errors for opening, reading and writing to and from files and for allocating memory.
    ///
    fn make(_: *std.Build.Step, prog: *std.Progress.Node) !void {
        _ = prog;
        std.debug.print("{s}\n", .{"hello world from stepp"});
    }
    ...
    ///
    pub fn create(builder: *std.Build, target: std.Build.ResolvedTarget) *RamdiskStep {
        const ramdisk_step = builder.allocator.create(RamdiskStep) catch unreachable;
        ramdisk_step.* = .{
            .step = std.Build.Step.init(.{.name = "zeta", .makeFn = make, .id = .custom, .owner = builder}),
            .builder = builder,
            .target = target,
            .files = &.{},
            .out_file_path = "none",
